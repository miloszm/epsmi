...
case class ImportMultiRequestV18(desc: String,
...
class BitcoindRpcExtendedClient(bitcoindInstance: BitcoindInstance, actorSystem: ActorSystem)
  ...
  val labelResultWithDefault: Reads[LabelResult] =
    (__ \ "name").readWithDefault("").and((__ \ "purpose").read[LabelPurpose])(LabelResult)

  def mapAddressesByLabelReadsWithDefault: Reads[Map[BitcoinAddress, LabelResult]] =
    Reads.mapReads[BitcoinAddress, LabelResult](s => JsSuccess(BitcoinAddress.fromString(s)))(labelResultWithDefault)
  ...
  override def getAddressesByLabel(label: String): Future[Map[BitcoinAddress, LabelResult]] = {
    bitcoindCall[Map[BitcoinAddress, LabelResult]]("getaddressesbylabel", List(JsString(label)))(
      mapAddressesByLabelReadsWithDefault
    )
  }
  ...
  override def testMempoolAccept(transaction: Transaction,
  ...
  /**
    ...
    */
  override protected def bitcoindCall[T](command: String,
...
//       logger.info(s"Payload: \n${Json.prettyPrint(payload)}")
  ...
  private def parseResult[T](result: JsResult[T], json: JsValue, printError: Boolean, command: String): T = {
