...
object MerkleProofOps {
  ...
  def descendMerkleTree(hashList: ListIter[String],
                        flags: ListIter[Boolean],
                        height: Int,
                        txCount: Int,
                        pos: Int): MerkleNode = {
    val flag = flags.next()
  ...
  def expandTreeElectrumFormatMerkleProof(node: TupleMerkleNode, accumulator: List[String]): List[String] = {
    val left  = node.left
    val right = node.right
    val acc1 = accumulator ++ (if (left.isTuple)
                                 expandTreeElectrumFormatMerkleProof(left.asInstanceOf[TupleMerkleNode], accumulator)
                               else Nil)
    val acc2 = acc1 ++ (if (right.isTuple)
                          expandTreeElectrumFormatMerkleProof(right.asInstanceOf[TupleMerkleNode], accumulator)
                        else Nil)
