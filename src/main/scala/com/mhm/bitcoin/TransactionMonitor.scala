...
/**
  ...
  */
class TransactionMonitorImpl(rpcCli: BitcoindRpcExtendedClient, nonWalletAllowed: Boolean)
  ...
  def buildAddressHistory(monitoredScriptPubKeys: Seq[String],
    ...
    val state =
      TransactionMonitorState(addressHistory = ah, deterministicWallets = deterministicWallets)
    ...
    @tailrec
    def insertTxsInHistory(state: TransactionMonitorState,
                           transactions: List[ListTransactionsResult],
                           obtainedTxids: Set[String]): (TransactionMonitorState, Set[String]) =
      transactions match {
        ...
        case tx :: xs =>
          if (isTxHistoryEligible(tx, obtainedTxids)) {
            ...
            val (outputScriptpubkeys, inputScriptpubkeys, txd) =
              getInputAndOutputScriptpubkeys(tx.txid.get)
            val shToAddOut =
              walletAddrScripthashes.intersect(outputScriptpubkeys.map(script2ScriptHash).toSet)
            val shToAddIn =
              walletAddrScripthashes.intersect(inputScriptpubkeys.map(script2ScriptHash).toSet)
            val shToAdd = shToAddIn ++ shToAddOut
            logger.trace(s"${shToAdd.size} scripthashes to add")
            if (shToAdd.isEmpty) insertTxsInHistory(state, xs, obtainedTxids)
            else {
              for (wal <- deterministicWallets) {
                val overrunDepths =
                  wal.haveScriptpubkeysOverrunGaplimit(outputScriptpubkeys)
                if (overrunDepths.nonEmpty)
                  throw new IllegalStateException(
                    s"not enough addresses imported, overrun depths=$overrunDepths, see transactionmonitor.py line 155"
                  )
              }
              val tx4HistoryGen =
                Tx4HistoryGen(tx.confirmations.get, tx.txid.map(_.hex).get, tx.blockhash)
              val newHistoryElement =
                generateNewHistoryElement(tx4HistoryGen, txd)
              val state1 = state.addHistoryItemForScripthashes(shToAdd.toSeq, newHistoryElement)
              val state2 =
                if (tx.confirmations.isDefined && tx.confirmations.get > 0 && tx.confirmations.get < ConfirmationsSafeFromReorg) {
                  state1.addReorganizableTx(
                    ReorganizableTxEntry(
                      optSha2Str(tx.txid),
                      tx.blockhash.map(_.hex),
                      newHistoryElement.height,
                      shToAdd.toSeq
                    )
                  )
                } else state1
    ...
    @tailrec
    def processListTransactions(skip: Int,
                                obtainedTxids: Set[String],
                                state: TransactionMonitorState): TransactionMonitorState = {
      val transactions =
        wrap(rpcCli.listTransactions("*", BATCH_SIZE, skip, includeWatchOnly = true), "listTransactions")
      logger.trace(
        s"obtained ${transactions.size} transactions (skip=$skip) ${transactions.map(_.txid.map(_.hex.substring(0, 4))).mkString("|")}"
      )
      val lastTx = if (transactions.nonEmpty) transactions.headOption else None
      val state2 =
        state.setLastKnownTx(lastTx.map(last => TxidAddress(optSha2Str(last.txid), optAddr2Str(last.address))))
      val (state3, newTxids) =
        insertTxsInHistory(state2, transactions.toList, Set())
    ...
    val state2 =
      processListTransactions(skip = 0, obtainedTxids = Set(), state).sortAddressHistory()
    val state3 = state2.initUnconfirmedTxs()
    logger.trace(
      s"finished buildAddressHistory, history size = ${state3.addressHistory.m.size}, last known = ${state3.lastKnownTx
        .map(_.txid.substring(0, 4))}"
    )
    state3.addressHistory.m.foreach {
      case (k, v) =>
  ...
  def getInputAndOutputScriptpubkeys(txid: DoubleSha256DigestBE): (Seq[String], Seq[String], RpcTransaction) = {
    ...
    val inputScriptpubkeys =
      txd.vin.filterNot(_.isInstanceOf[CoinbaseInput]).flatMap { inn =>
        val inputTransactionId =
          DoubleSha256DigestBE.fromHex(inn.previousOutput.txIdBE.hex)
        ...
        resultTry match {
          ...
          case Success(inputTransaction) =>
            ...
            Some {
              ...
              val inputDecoded =
                wrap(rpcCli.decodeRawTransaction(inputTransaction))
    ...
    logger.trace(
      s"finished getInputAndOutputScriptpubkeys with ${outputScriptpubkeys.size} output(s) and ${inputScriptpubkeys.size} input(s)"
    )
    ...
    if (tx.confirmations == 0) {
      val valueConfirmedPairs = txd.vin.flatMap { inn =>
        ...
        /**
          ...
          */
        Try(wrap(rpcCli.getTxOut(inn.previousOutput.txIdBE, inn.previousOutput.vout.toInt)))
          .fold(_ => None, utxo => Some(ValueConfirmedPair(utxo.value.toBigDecimal, utxo.confirmations > 0)))
      }
      val totalInputValue =
        valueConfirmedPairs.map(_.v).fold(BigDecimal(0))(_ + _)
      val unconfirmedInput =
        !valueConfirmedPairs.map(_.confirmed).forall(_ == true)
      val height = if (unconfirmedInput) -1 else 0
      val totalOut =
        txd.vout.map(_.value.toBigDecimal).foldLeft(BigDecimal(0))(_ + _)
      val fee = totalInputValue - totalOut
      HistoryElement(tx.txid, height, fee)
    } else {
      val blockHeader = wrap(
        rpcCli.getBlockHeader(tx.blockhashOpt.getOrElse(throw new IllegalArgumentException("blockhash missing")))
      )
  ...
  def checkForNewTxs(stateArg: TransactionMonitorState): TransactionMonitorState = {
    ...
    case class GetTrRes(containsKnown: Boolean,
                        newBegin: Int,
    ...
    @tailrec
    def getTransactions(attempt: Int,
                        maxAttempts: Int,
                        count: Int,
                        v: Vector[ListTransactionsResult],
                        lastKnownTx: Option[TxidAddress]): GetTrRes = {
      if (attempt == maxAttempts) GetTrRes(containsKnown = false, 0, v.size, v)
      else {
        val transactions =
          wrap(rpcCli.listTransactions("*", count, 0, includeWatchOnly = true), "listTransactions").reverse
        logger.trace(
          s"obtained ${transactions.size} transactions (skip=0) ${transactions.map(_.txid.map(_.hex.substring(0, 4))).mkString("|")}"
        )
        lastKnownTx match {
          case None =>
            GetTrRes(containsKnown = false, 0, transactions.size, transactions)
          case Some(ln) =>
            val found = transactions.zipWithIndex.find {
              case (t, _) =>
                optSha2Str(t.txid) == ln.txid && optAddr2Str(t.address) == ln.address
            }
            logger.trace(s"found last known ${ln.txid.substring(0, 4)} at index: ${found
              .map(_._2)} from among: ${transactions.map(_.txid.map(_.hex.substring(0, 4))).mkString("|")}")
            found match {
              case Some((_, recentTxIndex)) if count != transactions.size =>
                GetTrRes(containsKnown = true, 0, recentTxIndex, transactions)
              case _ =>
                getTransactions(attempt + 1, maxAttempts, count * 2, transactions, lastKnownTx)
            }
        }
      }
    }
    @tailrec
    def updateStateWithTransactions(state: TransactionMonitorState,
                                    txs: List[ListTransactionsResult]): TransactionMonitorState = {
      txs match {
        case Nil => state
        case tx :: xs =>
          ...
          val matchingScripthashes = (outputScriptpubkeys ++ inputScriptpubkeys)
            .map(script2ScriptHash)
            .filter(state.addressHistory.m.keySet.contains)
          val newHistoryElement =
            generateNewHistoryElement(Tx4HistoryGen(optConfirmations2Int(tx.confirmations), txid, blockhashOpt), txd)
          ...
          val newState3 = if (isReorganizable) {
            ...
            newState2.addReorganizableTx(
              ReorganizableTxEntry(txid, blockhashOpt.map(_.hex), newHistoryElement.height, matchingScripthashes)
            )
          } else newState2
    ...
    val getTrRes =
      getTransactions(0, maxAttempts, 2, Vector(), state.lastKnownTx)
    val resultState = if (getTrRes.isKnownOnly) {
      val newLastKnownTx =
        Some(getTrRes.transactions(getTrRes.knownBegin)).map { result =>
          TxidAddress(optSha2Str(result.txid), optAddr2Str(result.address))
        }
      val state1 =
        if (getTrRes.transactions.nonEmpty) state.setLastKnownTx(newLastKnownTx)
        else state
      state1
    } else {
      val newTxs =
        getTrRes.transactions.slice(getTrRes.newBegin, getTrRes.newEnd)
      ...
      updateStateWithTransactions(state.setLastKnownTx(newTxs.headOption.map { result =>
        TxidAddress(optSha2Str(result.txid), optAddr2Str(result.address))
      }), relevantTxs.toList)
    }
  ...
  def checkConfirmations(state: TransactionMonitorState): TransactionMonitorState = {
    @tailrec
    def processUnconfirmedTxs(state: TransactionMonitorState,
                              unconfirmedTxs: List[UnconfirmedTxEntry]): TransactionMonitorState =
      unconfirmedTxs match {
        case Nil => state
        case UnconfirmedTxEntry(txid, shs) :: xs =>
          ...
          if (tx.confirmations == 0) {
            ...
            val newState =
              state.deleteHistoryItemForScripthashes(shs, txid).addUpdatedScripthashes(shs)
            processUnconfirmedTxs(newState, xs)
          }
      }
    val newState = processUnconfirmedTxs(state, state.unconfirmedTxes.map {
      case (k, v) => UnconfirmedTxEntry(k, v)
    }.toList)
  ...
  def checkForReorganizations(state: TransactionMonitorState): TransactionMonitorState = {
    @tailrec
    def processReorganizableTxs(state: TransactionMonitorState,
                                reorganizableTxs: List[ReorganizableTxEntry]): TransactionMonitorState =
      reorganizableTxs match {
        case Nil => state
        case ReorganizableTxEntry(txid, blockhash, height, matchingShs) :: xs =>
          val reorganizable =
            ReorganizableTxEntry(txid, blockhash, height, matchingShs)
          val tx = wrap(rpcCli.getTransaction(DoubleSha256DigestBE.fromHex(txid)))
          if (tx.confirmations >= ConfirmationsSafeFromReorg) {
            ...
            val state2 = if (tx.confirmations == 0) {
              ...
              val state2 =
                if (!isOrphan) { // TODO not sure why details is a vector and if this category extraction is correct here
                  val txd = wrap(rpcCli.decodeRawTransaction(tx.hex))
                  val newHistoryElement =
                    generateNewHistoryElement(Tx4HistoryGen(tx.confirmations, tx.txid.hex, tx.blockhash), txd)
            ...
            processReorganizableTxs(
              state2
                .addUpdatedScripthashes(matchingShs)
                .removeReorganizable(reorganizable)
                .deleteHistoryItems(matchingShs, txid, height),
              xs
            )
          } else if (!tx.blockhash.map(_.hex).contains(blockhash)) {
            val block = wrap(
              rpcCli.getBlockHeader(tx.blockhash.getOrElse(throw new IllegalArgumentException("missing blockhash")))
            )
    ...
    val newState =
      processReorganizableTxs(state, state.reorganizableTxes.toList)
  ...
  def checkForUpdatedTxs(state: TransactionMonitorState): (Set[String], TransactionMonitorState) = {
    ...
    val updated =
      updatedScripthashes.filter(sh => state3.addressHistory.m.contains(sh) && state3.addressHistory.m(sh).subscribed)
  ...
  private def doGetAddressBalance(sh: String, history: Seq[HistoryElement]): Option[AddressBalance] = {
    ...
    @tailrec
    def go(hes: List[HistoryElement], utxos: Map[String, AmountConfirmations]): Map[String, AmountConfirmations] =
      hes match {
        case Nil => utxos
        case h :: xs =>
          ...
          val utxosToDel = txd.vin.map { input =>
            s"${input.previousOutput.txId}:${input.previousOutput.vout}"
          }
          go(xs, (utxos ++ utxosToAdd) -- utxosToDel)
      }
    val utxos = go(history.toList, Map())
    val confirmedBalance =
      utxos.values.filter(_.confirmations > 0).map(_.amount * BigDecimal(100000000)).sum
