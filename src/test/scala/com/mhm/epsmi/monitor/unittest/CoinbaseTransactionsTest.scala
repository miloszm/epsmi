...
class CoinbaseTransactionsTest extends FlatSpec with AddressHistoryAssertions {
  ...
  val (dummySpk1, containingBlockHeight1, dummyTx1) =
    createDummyFundingTx(masterId = 1000, coinbase = true, confirmations = 1) // bb8
  val (dummySpk2, containingBlockHeight2, dummyTx2) =
    createDummyFundingTx(masterId = 1001, coinbase = true, confirmations = 101) // bb9
  val (dummySpk3, containingBlockHeight3, dummyTx3) =
    createDummyFundingTx(masterId = 1002, coinbase = true, confirmations = 0) // bba
  val (dummySpk4, containingBlockHeight4, dummyTx4) =
    createDummyFundingTx(masterId = 1003, coinbase = true, confirmations = 1) // bbb
  val (dummySpk5, containingBlockHeight5, dummyTx5) =
    createDummyFundingTx(masterId = 1004, coinbase = true, confirmations = 101) // bbc
  val (dummySpk6, containingBlockHeight6, dummyTx6) =
    createDummyFundingTx(masterId = 1005, coinbase = true, confirmations = 0) // bbd

  val rpc = DummyBtcRpc(
    Seq(dummyTx1, dummyTx2, dummyTx3),
    Nil,
    Map(
  ...
  "check for updated tx" should "remove orphan tx from reorg and from history" in {
    ...
    val monitorState = monitor.buildAddressHistory(
      Seq(dummySpk1, dummySpk2, dummySpk3, dummySpk4, dummySpk5, dummySpk6),
      Seq(new DummyDeterministicWallet)
    )
