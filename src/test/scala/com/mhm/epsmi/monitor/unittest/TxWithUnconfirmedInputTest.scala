...
class TxWithUnconfirmedInputTest extends FlatSpec with AddressHistoryAssertions {
  ...
  "unconfirmed tx arrives with unconfirmed input, which then both confirm" should "update heights in history" in {
    ...
    val (dummySpk2, containingBlockHeight2, dummyTx2) =
      createDummyFundingTx(confirmations = 0, inputTxid = dummyTx1.txId, inputConfirmations = 0)

    val rpc = DummyBtcRpc(
      Nil,
      Seq(dummyTx1.vin, dummyTx2.vin),
    ...
    val monitorState2 =
      monitorState.subscribeAddress(script2ScriptHash(dummySpk1)).subscribeAddress(script2ScriptHash(dummySpk2))
