...
class TransactionMonitorStateTest extends FlatSpec {
  ...
  "add unconfirmed shs" should "add list of shs to an existing tx unconfirmed entry if it exists" in {
    ...
    val state = TransactionMonitorState(AddressHistory(Map()), unconfirmedTxes = unconfirmed).addUnconfirmedScripthases("tx1", Seq("sh3", "sh4"))
  ...
  "add unconfirmed shs" should "also work if tx was not in the list" in {
    ...
    val state = TransactionMonitorState(AddressHistory(Map()), unconfirmedTxes = unconfirmed).addUnconfirmedScripthases("tx2", Seq("sh3", "sh4"))
  ...
  "add unconfirmed shs" should "also work if the pre-existing shs list was empty" in {
    ...
    val state = TransactionMonitorState(AddressHistory(Map()), unconfirmedTxes = unconfirmed).addUnconfirmedScripthases("tx1", Seq("sh1", "sh2"))
  ...
  "add unconfirmed shs" should "eliminate duplicated shs" in {
    ...
    val state = TransactionMonitorState(AddressHistory(Map()), unconfirmedTxes = unconfirmed).addUnconfirmedScripthases("tx1", Seq("sh1", "sh1"))
  ...
  "update height for shs" should "update height in address history for all given shs" in {
    val addressHistory = AddressHistory(Map(
      "sh1" -> HistoryEntry(subscribed = true, Seq(HistoryElement("tx2", -1), HistoryElement("tx2", 0), HistoryElement("tx3", 500000), HistoryElement("tx3", 600000))),
      "sh2" -> HistoryEntry(subscribed = true, Seq(HistoryElement("tx3", 0), HistoryElement("tx3", 20), HistoryElement("tx3", 50), HistoryElement("tx3", 10))),
      "sh3" -> HistoryEntry(subscribed = true, Seq(HistoryElement("tx3", 1000), HistoryElement("tx2", 999), HistoryElement("tx3", 998), HistoryElement("tx2", 900))),
    ))
